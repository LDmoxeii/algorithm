...
class TopKFrequentTest {
    ...
    fun `test invalid inputs`() {
        // 空数组
        val emptyArrayException = assertThrows<IllegalArgumentException> {
            topKFrequent(intArrayOf(), 1)
        }
        assertEquals("数组不能为空", emptyArrayException.message)

        // k < 1
        val kTooSmallException = assertThrows<IllegalArgumentException> {
            topKFrequent(intArrayOf(1, 2, 3), 0)
        }
        assertEquals("k必须在1到数组长度之间", kTooSmallException.message)

        // k > 数组长度
        val kTooLargeException = assertThrows<IllegalArgumentException> {
            topKFrequent(intArrayOf(1, 2, 3), 4)
        }
        assertEquals("k必须在1到数组长度之间", kTooLargeException.message)
    }
