...
class CoinChangeTest {
    ...
    fun `test larger amounts`() {
        // 较大金额
        assertEquals(4, coinChange(intArrayOf(1, 2, 5, 10), 18)) // 10+5+2+1
        assertEquals(3, coinChange(intArrayOf(2, 5, 10, 25), 37)) // 25+10+2

        // 硬币面额不是按顺序的
        assertEquals(2, coinChange(intArrayOf(9, 6, 5, 1), 11)) // 6+5
    }
